...
-   Analyze the content of political speeches and texts to identify the key topics and themes [@miranda20211exploring].

-   Identify the most topical issues relating to the COVID-19 pandemic that were being discussed by South Africans on Twitter, as well as the impacts of these issues on the people's reactions to and compliance with the South African government's efforts in managing the disease [@mutanga2022].

-   Identify patterns in online discourse polarization and extremism, and the contributions and parameters thereof [@xu2020].

-   Study the relationship between political discourse and political outcomes [@brookes2019].

According to [@xu2020], LDA has been used in a variety of other ways in political science research. For example, it been used to: analyze the content of political advertising and study the relationship between political discourse and political violence.
...
words_to_remove <- c("government", "South Africa", "national",
                     "country", "south", "africa", "honourable", 
                     "people", 'development', 'ensure', 'public', 'economic',
...
```{r}
#| label: fig-topterms #| fig-cap: "Time series Plot of Average Sentiment using AFINN Lexicon Scoring System. Higher Values Represent more Positive Sentiment"  
top_terms <- speech_topics %>%  
  group_by(topic) %>%   
  top_n(20, beta) %>%   
  ungroup() %>%   
...
# some cleaning to make key variable (reviewId) usable 
    select(-document) %>% 
    spread(key = topic, value = gamma, sep = "_")) #  

speeches_gamma %>%  
  group_by(speechId) %>%  
  summarize(ntopic1 = sum(topic_1 > 0.000001), ntopic2 = sum(topic_2 > 0.000001), ntopic3 = sum(topic_3 > 0.000001), ntopic4 = sum(topic_4 > 0.000001))
```

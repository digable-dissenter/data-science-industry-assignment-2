...
Studies such as [@budiharto2018prediction] have suggested frameworks into using sentiment analysis on posts on relevant topics on some social media websites to predict election results. Sentiment analysis can also be used to determine the mood of the nation by monitoring announcements made by leaders of the country, including presidential candidates [@vo2017multi]. [@bringula2023youtube] analyzed comments on Youtube to analyze opinions from political vloggers about two presidential canditates. Sentiment analysis can be used to summarize political issues and current affairs [@bollen2011modeling]. [@miranda2021exploring] studied the the State of the Nation Addresses of the last 13 Philippine presidents and discovered that the speeches had huge focus towards economic development, enhancement of public services, and addressing issues faced by the nation.
...
In this paper, three different lexicons found in the 'tidyr' package were used: AFINN, bing and nrc. The AFINN (\href{http://www2.imm.dtu.dk/pubdb/pubs/6010-full.html}{Finn Årup Nielsen}) assigns a score to each word in the topic with -5 being the most negative and 5 being the most postive score. From \href{https:rh//www.cs.uic.edu/~liub/FBS/sentiment-analysis.html}{Bing Lui and collaborators} , the bing lexicon assigns each word in the tolkenized speeches as either negative or positive according to the lexicon association. From \href{http://saifmohammad.com/WebPages/NRC-Emotion-Lexicon.htm}{Saif Mohammad and Peter Turney}, the nrc lexicon assigns each word in the tolkenized speeches into categories of positive, negative, anger, anticipation, disgust, fear, joy, sadness, surprise, and trust. 
...
speech_tokens%>%
  anti_join(stop_words) %>%
  count(word) %>%
  with(wordcloud(word, n, max.words = 100, color = "random dark",
                 ordered.colors = TRUE))
```
...
speech_sentiment%>%select(Date, President, word, afinn_value)%>%na.omit%>% 
  group_by(Date, President) %>%summarise(Mean_Score = mean(afinn_value)) %>%ungroup() %>% 
